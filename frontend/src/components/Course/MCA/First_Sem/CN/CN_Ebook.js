...
export default function BasicTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const listRef = ref(storage, "MCA/First/First_Sem/CN/CN/Ebook");
  ...
  const download = async (name) => {
    try {
      const url = await getDownloadURL(
        ref(storage, `MCA/First/First_Sem/CN/CN/Ebook/${name}`)
      );
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/CN/CN/Ebook");

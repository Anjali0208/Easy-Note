...
export default function BasicTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const listRef = ref(storage, "MCA/First/First_Sem/CN/CN/CN_QP");
  ...
  const download = async (name) => {
    try {
      const url = await getDownloadURL(
        ref(storage, `MCA/First/First_Sem/CN/CN/CN_QP/${name}`)
      );
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/CN/CN/CN_QP");
    ...
    try {
      ...
      console.log("1");
      await addDoc(collection(firestore, "CN_QP"), {
        title: noteTitle,
        fileUrl: downloadURL,
      })
        .then(() => {

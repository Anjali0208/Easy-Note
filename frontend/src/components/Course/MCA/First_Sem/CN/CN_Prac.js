...
export default function BasicTable() {
  ...
  const download = async (name) => {
    try {
      const url = await getDownloadURL(
        ref(storage, `MCA/First/First_Sem/CN/CN/CN_Prac/${name}`)
      );
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/CN/CN/CN_Prac");
    ...
    try {
      ...
      console.log("1");
      await addDoc(collection(firestore, "CN_Prac"), {
        title: noteTitle,
        fileUrl: downloadURL,
      })
        .then(() => {
  ...
  return (
    <>
      <Container className="container">
        <Box margin="25px">
          <Typography variant="h5">Computer Networks Practical</Typography>
        </Box>

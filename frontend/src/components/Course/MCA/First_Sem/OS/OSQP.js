...
export default function BasicTable1() {
  ...
  useEffect(() => {
    const listRef = ref(storage, "MCA/First/First_Sem/OS/OS/OS_QP/");
    listAll(listRef)
      .then((res) => {
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/OS/OS/OS_QP");
    ...
    try {
      ...
      console.log("1");
      await addDoc(collection(firestore, "OS_QP"), {
        title: noteTitle,
        fileUrl: downloadURL,
      })
        .then(() => {
  ...
  const download = (name) => {
    getDownloadURL(ref(storage, `MCA/First/First_Sem/OS/OS_qp/${name}`))
      .then((url) => {
  ...
  return (
    <>
      <Container className="container">
        <Box margin="25px">
          <Typography variant="h5">Operating System Question Papers</Typography>
        </Box>

...
export default function BasicTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const listRef = ref(storage, "MCA/First/First_Sem/OS/OS/Ebook");
  ...
  const download = async (name) => {
    try {
      const url = await getDownloadURL(
        ref(storage, `MCA/First/First_Sem/OS/OS/Ebook/${name}`)
      );
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/OS/OS/Ebook");
  ...
  return (
    <>
      <Container className="container">
        <Box margin="25px">
          <Typography variant="h5">Operating System Ebook</Typography>
        </Box>

...
export default function BasicTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const listRef = ref(storage, "MCA/First/First_Sem/DBMS/DBMS/UserNotes");
  ...
  const download = async (name) => {
    try {
      const url = await getDownloadURL(
        ref(storage, `MCA/First/First_Sem/DBMS/DBMS/DBMS_Prac/${name}`)
      );
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/DBMS/DBMS/DBMS_Prac");
    ...
    try {
      ...
      console.log("1");
      await addDoc(collection(firestore, "DBMS_Prac"), {
        title: noteTitle,
        fileUrl: downloadURL,
      })
        .then(() => {
  ...
  return (
    <>
      <Container className="container">
        <Box margin="25px">
          <Typography variant="h5">DBMS Practical</Typography>
        </Box>

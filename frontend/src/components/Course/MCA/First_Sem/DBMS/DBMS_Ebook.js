...
export default function BasicTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const listRef = ref(storage, "MCA/First/First_Sem/DBMS/DBMS/Ebook");
  ...
  const download = async (name) => {
    try {
      const url = await getDownloadURL(
        ref(storage, `MCA/First/First_Sem/DBMS/DBMS/Ebook/${name}`)
      );
  ...
  const handleNoteUpload = async () => {
    const storageRef = ref(storage, "MCA/First/First_Sem/DBMS/DBMS/Ebook");
  ...
  return (
    <>
      <Container className="container">
        <Box margin="25px">
          <Typography variant="h5">Discrete Structure Ebook</Typography>
        </Box>
